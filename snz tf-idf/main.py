...
def process_document(doc, df, N, vocab):
   ...
   """
    ...
    tf_idf = get_tf_idf_dict_for_doc(doc, df, N)
...
def get_tf_idf_dict_for_doc(doc, df, N):
    if isinstance(doc, str):
        words = get_words(doc)
    else:
        words = doc
...
def process_document_with_words(doc, df, N, vocab):
    tf_idf = get_tf_idf_dict_for_doc(doc, df, N)
    # list of tuples (word, tf_idf)
    vector_with_words = calc_vector_with_words(tf_idf, vocab)
    return vector_with_words


# from tf_idf import get_vocabulary, get_words, pearson, rank_documents, cosine, create_dataset, build_tree, process_document_wi
...
if __name__ == '__main__':
    ...
    keywords = sorted(process_document_with_words(test_doc, df, N, vocab), key=lambda x: x[1], reverse=True)

    test_keywords = [item[0] for item in keywords if item[1] > 0]
    recnik = {}
    dok = 0
    for item in documents:
        vector = process_document_with_words(item, df, N, vocab)
        vec_elements = [x[0] for x in vector if x[1] > 0]
        zaednicki = []

        common = []

        counter = 0
        for word in vec_elements:
            for word2 in test_keywords:
                if word == word2:
                    counter += 1
                    common.append(word2)
        zaednicki.append(common)

        recnik[dok] = counter
        dok += 1

    result = sorted(recnik.items(), key=lambda x: x[1], reverse=True)[:4]

    for item in result:
        print(f"Dokument #{item[1]}: ")