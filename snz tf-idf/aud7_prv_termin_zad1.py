...
from tf_idf import cosine, pearson, rank_documents
...
if __name__ == '__main__':
    ...
    #############
    # text = data[4]
    # res_cos = rank_documents(text, data, cosine)[1:]
    #########
    res_cos = rank_documents(text, data, cosine)
    res_pear = rank_documents(text, data, pearson)
    flag = True
    for i in range(4):
        if res_pear[i][1] != res_cos[i][1]:
            flag = False
        print(f"Rang po slicnost:{i} Cosine-Document #{res_cos[i][1]} Pearson-Document #{res_pear[i][1]}")

    print(f"Dvete metriki davaat ist rezultat:{flag}")
