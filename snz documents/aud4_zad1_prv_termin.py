...
def get_words_with_ignore(doc, words_to_ingore=None):
    ...
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            if words_to_ignore is not None and word.lower() not in words_to_ingore:
...
if __name__ == '__main__':
    ...
    c2 = NaiveBayes(lambda x: get_words_with_ignore(x, words_to_ingore=words_to_ignore))
    ...
    # Branje 2 - Presmetka na verojatnosti za sekoja od klasite
    print(c1.calculate_document_probability_in_class(test_dokument, 'sport'))
    print(c1.calculate_document_probability_in_class(test_dokument, 'science'))
