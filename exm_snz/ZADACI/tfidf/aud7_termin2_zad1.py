...
if __name__ == '__main__':
    ...
    from tf_idf import rank_documents, pearson, cosine

    documents = [t[0] for t in train_data]

    result_pearson = rank_documents(text, documents, pearson)[:4]
    print(result_pearson)
    result_cosine = rank_documents(text, documents, cosine)[:4]
    print(result_cosine)

    counter = 0

    for i, r1, r2 in zip(range(4), result_pearson, result_cosine):
        print(f'Rang po slicnost: {i} Cosine-Document #{r2[1]}, Pearson-Document #{r1[1]}')
        if r1[1] == r2[1]:
            counter += 1
