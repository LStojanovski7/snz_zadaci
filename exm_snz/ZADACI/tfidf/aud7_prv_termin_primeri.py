...
from tf_idf import get_vocabulary, process_document, rank_documents, cosine, pearson, calculate_document_frequencies

if __name__ == '__main__':
    data = [x[0] for x in train_data]
    df = calculate_document_frequencies(data)
    print(df['movie'])
    voc = get_vocabulary(data)
    N = len(data)
    vec1 = process_document(data[0], df, N, voc)
    # print(voc)
    vec2 = process_document(data[1], df, N, voc)
    print(cosine(vec1, vec2))
    print(pearson(vec1, vec2))
    print(rank_documents(data[0], data[1:], pearson))
    print(rank_documents(data[0], data, cosine)[1:])
    #####################
    res = rank_documents(data[0], data[1:], pearson)
    print(data[1:][res[0][1]])
    ######################
    res = rank_documents(data[0], data, pearson)[1:]
    print(data[res[0][1]])
