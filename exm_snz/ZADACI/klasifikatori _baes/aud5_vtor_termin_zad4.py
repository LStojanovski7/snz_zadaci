...
if __name__ == '__main__':
    ...
    klasa = c.classify_document(recenica)
    print(klasa)
    verojatnost = c.get_category_probability_for_document(recenica, klasa)

    words = list(sorted(get_words(recenica)))

    for word in words:
        print(round(c.get_feature_per_category_probability(word, 'science'), 5))
        ver = c.weighted_probability(word, 'science', c.get_feature_per_category_probability)
        print(round(ver, 5))

        print(round(c.get_feature_per_category_probability(word, 'sport'), 5))
        ver2 = c.weighted_probability(word, 'sport', c.get_feature_per_category_probability)
        print(round(ver2, 5))
    print(klasa, round(log2(verojatnost), 5))
