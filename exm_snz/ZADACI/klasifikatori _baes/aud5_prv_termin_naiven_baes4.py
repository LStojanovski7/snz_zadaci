...
if __name__ == "__main__":
    ...
    ver = c.get_category_probability_for_document(recenica, klasa)
    print(round(ver, 5))

    zborovi = list(sorted(get_words(recenica)))

    for zbor in zborovi:
        print(zbor, 'science',
              round(c.weighted_probability(zbor, 'science', c.get_feature_per_category_probability), 5))
        print(zbor, 'sport', round(c.weighted_probability(zbor, 'sport', c.get_feature_per_category_probability), 5))

    print(klasa, round(log2(ver), 5))
