...
def get_words_emoticons(x, emoticons):
    words = set()
    for word in re.split('\\W+', x):
        if 2 < len(word) < 20:
            words.add(word.lower())
    for element in x.lower().split(' '):
        if element in emoticons:
            words.add(element)
    return words


def helper_function(doc):
    return get_words_emoticons(doc, emoticons)
...
if __name__ == '__main__':
    ...
    c2 = NaiveBayes(helper_function)

    for x in data:
        c1.train(x[1], x[0])
        c2.train(x[1], x[0])

    klasa1 = c1.classify_document(test_sample[0])
    klasa2 = c2.classify_document(test_sample[0])
