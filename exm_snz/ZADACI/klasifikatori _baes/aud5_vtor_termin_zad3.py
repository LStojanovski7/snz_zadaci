...
def helper(doc):
    return get_words_with_emoticons(doc, emoticons)


if __name__ == '__main__':
    sample_ind = int(input())
    test_sample = data.pop(sample_ind)

    c1 = NaiveBayes(get_words)
    c2 = NaiveBayes(helper)
    # c2 = NaiveBayes(lambda doc: get_words_with_emoticons(doc, emoticons))
    ...
    print(test_sample[1])
    print(f'Vistinska klasa: {test_sample[0]}')

    klasa1 = c1.classify_document(test_sample[1])
    print(f'Klasa so Naiven Baesov klasifikator 1: {klasa1}')

    klasa2 = c2.classify_document(test_sample[1])
    print(f'Klasa so Naiven Baesov klasifikator 2: {klasa2}')
