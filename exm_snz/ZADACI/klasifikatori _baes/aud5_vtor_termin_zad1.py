...
if __name__ == '__main__':
    sample = input()
    training_set1 = train_data[:int(len(train_data) * 0.4)]
    training_set2 = train_data[int(len(train_data) * 0.4):int(len(train_data) * 0.8)]
    test_data = train_data[int(len(train_data) * 0.8):]

    c1 = NaiveBayes(get_words)
    c2 = NaiveBayes(get_words)

    for x in training_set1:
        c1.train(x[0], x[1])

    for x in training_set2:
        c2.train(x[0], x[1])
    ...
    for x in test_data:
        ...
        vistinska_klasa = x[1]
        if klasa1 == klasa2 and vistinska_klasa == klasa1:
            brojach += 1

    print(brojach / len(test_data))
