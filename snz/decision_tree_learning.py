...
# ----------------------------------------------------------------------------------------------------------
#                                      Custom defs below this point
# ----------------------------------------------------------------------------------------------------------

def getMaxPrediction(classification: dict):

    return max(classification.items(), key= lambda x : x[1])[0]

def print_tree_custom(tree, indent='', level=0):
    ...
    if tree.results:
        ...
        # Се печатат True гранките, па False гранките
        print(indent + 'T-> ', end='')
        print_tree_custom(tree.tb, indent + '  ', level + 1)
        print(indent + 'F-> ', end='')
        print_tree_custom(tree.fb, indent + '  ', level + 1)

def removeColumn(dataset: list, column: int):
